...
| STRAT-02 | plan only shows the path to win, no failure anatomy | **Invert, always invert** — define non-X and disconfirmers before the forward plan (↔ ml [[FAIR-02]](ml-systems.md)/[[FAIR-03]](ml-systems.md) disaggregated evidence before deploy; ↔ ml [[HITL-05]](ml-systems.md) agreement is not truth before the label commits; ↔ ux [[INT-07]](interaction-ux.md) preview before costly commit; ↔ ml [[EVAL-01]](ml-systems.md)/[[TEST-03]](engineering.md) baseline and characterization before the new contract) | Where do we die, and what evidence would kill this thesis? | B·s | poor-charlies-almanack ch-5 |
| STRAT-03 | team enters a domain where the edge boundary is unknown | **Circle of competence** — yes / no / too-tough baskets; no fake fluency (↔ epi [[NDM-01]](epistemics.md) trust expertise only when validity and feedback hold; ↔ ux [[HAI-05]](interaction-ux.md) do not name capability beyond the tested envelope; ↔ ml [[CAL-02]](ml-systems.md) unknown is a valid result; ↔ ml [[EMB-04]](ml-systems.md) de-emphasise the identity-void rather than force a face; ↔ ml [[HITL-07]](ml-systems.md) escalate the unsure item to a human) | If the next hard question comes, are we Planck or the chauffeur? | B·s | poor-charlies-almanack ch-3 |
| STRAT-04 | rare clear edge appears with favorable odds | **Load the boat** — bet seldom, bet big when mispriced; pair with [[OPS-02]](business-marketing.md) sit-on-your-hands | If we only get 20 punches lifetime, is this punch-worthy? | S·s | poor-charlies-almanack ch-8 |
...
| STRAT-14 | big reversible decision frozen by dread | **Fear-setting** — named worst cases become repair plans; vague fear freezes (↔ epi [[NDM-02]](epistemics.md) pre-mortem names the failure modes before commit; ↔ eng [[RLSE-08]](engineering.md) write the recovery before ship) | Worst case, repair path, and the 10-year cost of inaction? | S·s | four-hour-workweek ch-3 |
...
| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| PROD-01 | feature request without an outcome metric | **Outcome over output** — ship only what changes valuable behavior (↔ [[OPS-01]](business-marketing.md) you get the incentive you pay for, not the outcome you meant; ↔ eng [[TEST-11]](engineering.md) stability over gameable coverage; ↔ ux [[UXR-03]](interaction-ux.md) a metric that cannot decide the goal is the wrong number; ↔ writ [[WRIT-28]](writing.md) prestige proxies are not named effects) | What human behavior must change, by how much? | B·p | lean-ux ch-3 |
...
| PROD-04 | experiment fails but the team loves the design | **Falsification wins** — kill or pivot; taste isn't evidence (↔ [[STRAT-02]](business-marketing.md) define the disconfirmers before the plan; ↔ eng [[DBG-06]](engineering.md)/[[DBG-11]](engineering.md) a fix is real only when the cause's absence kills the failure; ↔ sec [[SECD-07]](security.md) threat-model the ways it dies before buying controls; ↔ epi [[FORE-03]](epistemics.md) score the forecast, do not reframe hits) | What result would make us wrong — and did we see it? | B·p | lean-ux ch-12 |
| PROD-05 | low evidence, high-fidelity plan | **Truth Curve** — investment proportional to market evidence (↔ ux [[UXR-01]](interaction-ux.md) the method must be able to support the claim's level; ↔ [[STRAT-02]](business-marketing.md) evidence before the commitment; ↔ ml [[PROV-02]](ml-systems.md) model card before release; ↔ ux [[INT-07]](interaction-ux.md) preview before the costly commit) | What evidence justifies this fidelity? | S·p | lean-ux ch-12 |
...
| PROD-09 | feature ignores hard channel constraints (length, hook, review rules) | **Format-first construction** — write the channel contract before the components (↔ [[CLM-05]](business-marketing.md) three hostile readers of the claim; ↔ eng [[RLSE-09]](engineering.md) plain-English expert gate; ↔ sec [[SECD-01]](security.md) write the policy before the mechanism; ↔ eng [[AGT-08]](engineering.md) a guard rejection is the named contract to satisfy) | What will the gatekeeper fade, skip, or demote? | B·p | klf-the-manual ch-4 |
...
| PROD-12 | builders never meet the operational user on the real workflow | **Builder–user proximity** — continuous first-hand contact with the person who runs the product on their worst day; a requirements doc is not a substitute for getting out of the building (↔ a11y [[A11Y-23]](accessibility.md): the worst-day user includes the AT user) | Would a field user trust this UI on their worst day? | S·p | lean-ux ch-7 |
...
| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| AIPX-01 | "we need a model for this" before rules tried | **Heuristic before ML** — ship the deterministic baseline first (↔ ml [[EVAL-01]](ml-systems.md) without the heuristic baseline the model metric is uninterpretable; ↔ [[STRAT-02]](business-marketing.md) evidence before the commitment) | Can a maintainable rule deliver the value? | B·p | building-ml-powered-applications ch-1 |
| AIPX-02 | offline metric up, user acceptance flat | **Product-metric sovereignty** — offline scores are proxies; the user behavior is the metric (↔ [[OPS-01]](business-marketing.md) you get the number you pay for; ↔ [[PROD-01]](business-marketing.md) ship the outcome, not the offline score) | Which user behavior proves this model helped? | B·p | building-ml-powered-applications ch-2 |
| AIPX-03 | no accept/edit instrumentation on model output | **Instrument the flywheel** — production use is the label source (↔ ml [[HITL-01]](ml-systems.md) each train run must consume a new human-label batch; ↔ eng [[DATA-14]](engineering.md) no dual writes of the truth; ↔ ux [[HAI-02]](interaction-ux.md) correction must reach the source; ↔ ml [[SERVE-04]](ml-systems.md) no post-hoc correction cascade; ↔ ux [[INT-09]](interaction-ux.md) edit is a reversible command on the real data; ↔ eng [[REF-09]](engineering.md) a derived field that skips the source drifts) | Where does the next training label come from? | B·p | building-ml-powered-applications ch-4 |
| AIPX-04 | only aggregate accuracy at ship review | **Slice before ship** — per-cohort and failure-mode checks (↔ ml [[FAIR-02]](ml-systems.md) intersectional slices expose the compound cell the aggregate cleared; ↔ [[OPS-01]](business-marketing.md) the headline number is not the outcome; ↔ [[PROD-01]](business-marketing.md) outcome over the cleared aggregate) | On which user/content slice would we be embarrassed? | B·p | building-ml-powered-applications ch-5 |
...
| AIPX-07 | advice/description feature tuned for coverage | **Precision-first guidance** — wrong advice costs more than silence (↔ ml [[CAL-02]](ml-systems.md) abstention is safer than a forced wrong class; ↔ [[STRAT-03]](business-marketing.md) too-tough is a designed basket; ↔ eng [[RLSE-04]](engineering.md) empty/error/offline are designed states) | What happens to trust the first time we're wrong? | S·p | building-ml-powered-applications ch-7 |
| AIPX-08 | generative output presented as complete | **Output is not the answer** — design for interpretation, edit, and next action (↔ ux [[HAI-01]](interaction-ux.md) a machine claim without inspectable evidence is uncalibrated reliance) | Will the user treat this as verified truth? | B·p | designing-ai-interfaces ch-5 |
| AIPX-09 | objective claim drives real action | **Secondary verification for stakes** — high-stakes outputs need a human or external check layer (↔ sec [[SEC-05]](security.md) human approves before the side effect executes; ↔ eng [[DATA-18]](engineering.md) check-then-act under weak isolation is the irreversible side; ↔ a11y [[A11Y-25]](accessibility.md) retrofit costs more than the hedge at write time) | Can a wrong accept cause irreversible harm? | B·p | designing-ai-interfaces ch-5 |
...
| AIPX-11 | AI fluent but possibly wrong | **User as error detector** — afford double-check, edit, and source paths when the system can't self-flag (↔ ux [[HAI-01]](interaction-ux.md) every machine claim links to inspectable evidence) | Is failure detectable by the system, or only by the user? | B·p | designing-ai-interfaces ch-4 |
| AIPX-13 | humanlike AI in a commercial surface | **Disclose AI early** — synthetic nature labeled as an understanding aid (↔ ux [[HAI-14]](interaction-ux.md) label AI content that could pass as human; ↔ grph [[GRPH-18]](graph-theory.md) a named similarity boundary the data never enforced; ↔ sec [[WEB-21]](security.md) ambient authority the eye never chose; ↔ eng [[NAME-03]](engineering.md) a name that promises a type the code does not hold) | Could a user reasonably think this is a human? | B·g | designing-ai-interfaces ch-5 |
| AIPX-14 | perfect accuracy assumed in UX copy | **Design for imperfect models** — confidence gates and human override in the flow (↔ ux [[HAI-04]](interaction-ux.md) user must be able to stop and steer; ↔ ml [[CAL-02]](ml-systems.md) below-threshold is a designed state; ↔ [[STRAT-03]](business-marketing.md) too-tough over fake fluency; ↔ eng [[RLSE-04]](engineering.md) undesigned empty/error is a bug) | What is the worst wrong output a user might see? | B·p | building-ml-powered-applications ch-2 |

## 4. Go-to-Market & Distribution

| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| GTM-01 | "users said they'd buy" without money | **Ask them to buy** — only paid intent validates (↔ ux [[UXR-03]](interaction-ux.md) a metric that cannot decide the goal is the wrong number; stated intent cannot decide paid demand; ↔ [[STRAT-02]](business-marketing.md) evidence before the inventory commitment) | What costly action (purchase, preorder, deposit) proves demand? | B·g | four-hour-workweek ch-10 |
...
| GTM-04 | plan waits on a major platform/partner for distribution | **Distribution-as-product via service stack** — rent the infrastructure under your own label (↔ eng [[REF-15]](engineering.md) every hard-coded dependency is a forfeited exit) | Can we rent the muscle without selling the company? | S·g | klf-the-manual ch-10 |
...
| GTM-06 | heat is high but the ranked/instrumented number is flat | **Resource the ranking nodes** — spend where the number is actually typed, scanned, counted (↔ [[OPS-01]](business-marketing.md) pay for the node that records the outcome; ↔ [[PROD-01]](business-marketing.md) heat is not valuable behavior change) | Where is the number actually recorded? | B·g | klf-the-manual ch-12 |
...
| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| OPS-01 | KPI/comp/pricing plan can be gamed or pays for the wrong output | **Incentives first** — never think about anything else before incentives are right (↔ ml [[COST-01]](ml-systems.md) if the target is not an owned SLI, no one is paying for it; ↔ ml [[COST-04]](ml-systems.md) pay per accepted output, not busy hardware; ↔ epi [[FORE-03]](epistemics.md) unscored forecasts pay for narrative confidence; ↔ eng [[OBS-07]](engineering.md) page on user symptoms, not a CPU proxy; ↔ eng [[PERF-03]](engineering.md) green averages that omit the slow tail; ↔ sec [[SEC-12]](security.md) clean accuracy is not a backdoor probe) | What behavior does this pay for at 2 a.m. with no witnesses? | B·o | poor-charlies-almanack ch-10 |
...
| OPS-08 | founder is the bottleneck after sales exist | **Management by absence** — written authority and rules beat heroics (↔ [[OPS-13]](business-marketing.md) who/what/when/where/why or rewrite; ↔ eng [[AGT-07]](engineering.md) externalize at discovery before compaction; ↔ des [[BRND-12]](design-aesthetics.md) systemize taste so the brand outlives the founder) | What written rule would let this run without me for 30 days? | S·o | four-hour-workweek ch-11 |
...
| OPS-13 | order/plan omits rationale | **Braun why-rule** — who/what/when/where/why or rewrite (↔ eng [[ARCH-07]](engineering.md) context, decision, and consequences must outlive the author) | Why would a smart skeptic comply? | S·o | poor-charlies-almanack ch-6 |
...
| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| BOOT-01 | zero-capital brand building | **Own before polish** — the first assets are rights, name, and a direct contact path; polish comes after ownership (↔ eng [[REF-15]](engineering.md) every hard-coded dependency is a forfeited exit) | If the platform dies tomorrow, what residual remains? | B·s | porn-work ch-3 |
...
| BOOT-07 | one platform carries the revenue | **Hostile-landlord assumption** — multi-home the masters and the rails before you need to (↔ [[GTM-04]](business-marketing.md) rent muscle under your own label; ↔ sec [[SEC-04]](security.md) least-privilege agency so a jailbreak reaches little) | Can a ToS change kill >50% of revenue in a week? | B·s | porn-work ch-9 |
| BOOT-08 | follower counts as KPIs | **Payers per 1k followers** — optimize paid conversion, not reach (↔ [[OPS-01]](business-marketing.md) the reach number is not the paid outcome; ↔ [[PROD-01]](business-marketing.md) payers are the behavior change) | How many payers per thousand? | S·g | porn-work ch-9 |
...
| CLM-02 | stronger claim proposed because it converts better | **The cost asymmetry is absolute** — the safe side costs a softer verb or a hedge; crossing costs orders of magnitude; no aggressive claim is worth the exposure (↔ [[AIPX-09]](business-marketing.md) high-stakes accept needs a check layer; ↔ eng [[RLSE-08]](engineering.md) write the rollback before ship; ↔ eng [[DATA-17]](engineering.md) weak isolation looks free and costs integrity later; ↔ sec [[SEC-04]](security.md) narrow scope now, not after the jailbreak) | What does the cautious phrasing actually lose? | B·g | product-deploy-agents-fields ch-7 |
| CLM-03 | privacy policy (or ToS) and implementation drift apart | **The policy is a contract** — every statement is a promise; audit claim-by-claim; usually the policy is the stale side; never ship them in contradiction (↔ a11y [[A11Y-22]](accessibility.md) one failing step falsifies the claim; ↔ [[AIPX-13]](business-marketing.md) synthetic labeled as human is a boundary the system does not hold; ↔ sec [[SEC-01]](security.md) trust that changes at every crossing; ↔ eng [[ARCH-13]](engineering.md) a property left to "everyone will be careful" is only perceived; ↔ ux [[HAI-14]](interaction-ux.md) disclose the synthetic hand; ↔ eng [[NAME-03]](engineering.md) names that lie about the type) | Which policy sentence does this diff falsify? | B·o | product-deploy-agents-fields ch-7 |
| CLM-04 | "encrypted", "compliant", "secure", "accessible" as bare adjectives | **Adjectives are not evidence** — the full claim is mechanism + location + failure behavior; anything less is assertion (↔ ux [[HAI-08]](interaction-ux.md) uncertainty at the decision granularity; ↔ a11y [[A11Y-22]](accessibility.md) for "accessible") | What completes this claim? | B·o | product-deploy-agents-fields ch-7 |
| CLM-05 | copy reviewed only by its author's intent | **Three hostile readers** — the regulator, the platform review team, and the plaintiff's attorney in three years; read literally, not charitably (the platform reviewer is [[BOOT-07]](business-marketing.md)'s hostile landlord wearing a badge; ↔ [[STRAT-02]](business-marketing.md) the claim must survive its disconfirmers before it ships; ↔ eng [[RLSE-09]](engineering.md) plain-English expert gate, not liability-shifting jargon) | Who profits from reading this against us, and what do they find? | S·g | product-deploy-agents-fields ch-7 |

## 9. Cross-source tensions worth keeping

- **Conviction vs falsification**: [[STRAT-07]](business-marketing.md) (Karp: don't let the crowd veto) ↔ [[PROD-04]](business-marketing.md) (Lean UX: evidence kills darlings). Resolution by object: *values and positioning* run on conviction; *features and funnels* run on falsification.
- **Novelty vs precision**: [[GTM-08]](business-marketing.md) (KLF: novelty wins first exposure) ↔ [[AIPX-07]](business-marketing.md) (Ameisen: precision-first for advice products). First exposure can be novel; the *output* can never be sloppy in a trust product.
- **Niche membership vs consequential problems**: [[GTM-03]](business-marketing.md) (Ferriss: sell to a tribe you belong to) ↔ [[STRAT-06]](business-marketing.md) (Karp: work on what matters). A rare wedge satisfies both; hunt for it.
- **Novelty vs the regulatory line**: [[GTM-08]](business-marketing.md) (KLF: novelty beats craft at first exposure) ↔ [[CLM-02]](business-marketing.md) (Fields: the cost asymmetry is absolute). The hook can be as loud as the campaign wants; the claim inside it stays on the safe side of the line. The novelty lives in the packaging, not in the claim.
- **Channel contract vs claims review**: [[PROD-09]](business-marketing.md) (KLF: write the gatekeeper's contract before the components) and [[CLM-05]](business-marketing.md) (three hostile readers) are the same move at different stakes: App Store guidelines, FDA wellness lines, and radio-edit rules are all channel contracts read by someone paid to find violations. KLF learned it from playlist committees; Fields from app review.
- **Boring releases vs loud launches**: [[GTM-08]](business-marketing.md)/[[GTM-13]](business-marketing.md) stunts ↔ eng [[RLSE-07]](engineering.md) (phased rollout, stop criteria). The marketing moment can be theatrical, but the rollout percentage is set by the monitoring plan, never by the stunt calendar.
