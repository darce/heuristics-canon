...
| GRPH-22 | Production clustering still merges distinct entities or splits one entity | **Replace hard-τ-plus-components with partition-plus-verification** — keep only high-precision edges, cut with a constrained objective ([[GRPH-19]](graph-theory.md)), then re-verify the residual cross-group bridges ([[GRPH-21]](graph-theory.md)); gate on pair precision/recall at the operating edge set, not component purity | What is pair-level precision at the operating edge set, and which bridges are re-verified? | S·w | graph-classification-and-clustering |
| GRPH-23 | A similarity graph built with one global cosine/L2 threshold | **Calibrate the edge policy per quality stratum** — pose, age, and capture quality shift the similarity distribution, so one global τ over-links easy negatives and under-links hard positives; use adaptive or learned thresholds per regime (edge construction itself: see [[GRPH-17]](graph-theory.md)) | Is the edge policy validated across quality strata, or only on clean high-quality pairs? | S·w | spectral-clustering-and-biclustering |
