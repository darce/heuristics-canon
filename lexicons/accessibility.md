...
| Src | source slug, resolved in the public repo's `SOURCES.md`; the distillation behind it stays in the private research corpus |
...
Sources: W3C WAI, *WCAG 2.2* (© W3C); WebAIM, *The WebAIM Million 2026*; AODA
rows from practitioner summaries, with their limits noted in the distillation.
...
- **Contrast is designed upstream**: design [[COL-04]](design-aesthetics.md) (value architecture before hue) makes [A11Y-01] pass by construction; [A11Y-01] is the measurable readout that [[COL-04]](design-aesthetics.md) was actually followed. A team failing contrast in review skipped a design step, not an engineering one.
- **The state matrix has an accessibility dimension**: eng [[RLSE-04]](engineering.md) (undesigned state is a bug) × [A11Y-24]: a state isn't designed until its focus order and announcements are. PDA's UX lens and WCAG audit the same seven states from two threat models.
- **Claims discipline transfers whole**: [A11Y-22] is [[CLM-01]](business-marketing.md)/[[CLM-04]](business-marketing.md) instantiated: "accessible" is an adjective-without-mechanism exactly like "encrypted", and the EAA/ADA give it the same enforcement asymmetry [[CLM-02]](business-marketing.md) describes.
...
- **Platform floor vs spec floor**: [A11Y-14] (24px) vs platform HIG (44pt/48dp): always the stricter; the same "stricter-of-two-gatekeepers" logic as biz [[PROD-09]](business-marketing.md) channel contracts.
