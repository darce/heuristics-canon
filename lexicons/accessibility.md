...
| --- | --- | --- | --- | --- | --- |
| A11Y-01 | text or UI colors set or changed in a diff/mockup | **Contrast floors (1.4.3/1.4.11)** — text ≥4.5:1, large text and UI component states ≥3:1; logos/incidental exempt; fails on 83.9% of pages (↔ eng [[UI-02]](engineering.md?plain=1#L366) owns the token mechanics; ↔ design [[COL-04]](design-aesthetics.md?plain=1#L76): a value-ranked palette passes free) | Does every pair pass its ratio on every real ground? | B·w | wcag22-accessibility ch-2 |
...
| A11Y-06 | status, trend, series, or validity distinguished by hue alone | **Second channel always (1.4.1)** — pair color with icon/label/weight/pattern (↔ eng [[UI-02]](engineering.md?plain=1#L366) — same rule, this row adds the criterion number and scan trigger) | Does the signal survive greyscale? | B·r | wcag22-accessibility ch-2 |
...
| A11Y-13 | sticky header, toast, or cookie banner added to layout | **Focus not obscured (2.4.11)** — the focused element is never entirely hidden behind author content (↔ eng [[WEB-21]](security.md?plain=1#L111) an overlay that steals the interaction surface from what the user can see) | Where is keyboard focus when the banner is up? | S·r | wcag22-accessibility ch-6 |
...
| A11Y-18 | legal, financial, or data-destroying submission flow | **Reversible, checked, or confirmed (3.3.4)** — at least one of undo / validation-with-correction / confirmation step (↔ ux [[INT-07]](interaction-ux.md?plain=1#L117) preview before a costly commit; ↔ ux [[INT-09]](interaction-ux.md?plain=1#L119) reversible command stack) | Can one mis-tap destroy something permanent? | B·r | wcag22-accessibility ch-4 |
| A11Y-19 | login/auth or multi-step form in a diff | **No cognitive gate (3.3.7/3.3.8)** — paste/autofill are the mechanisms that usually satisfy 3.3.8; blocking them with no alternative method fails; data entered once is never re-typed in the same process (↔ ux [[COG-02]](interaction-ux.md?plain=1#L87) recognition and choosers beat pure recall) | Does a password manager complete this unaided? | S·r | wcag22-accessibility ch-6 |
...
| ID | Trigger | Rule | Answers | T·P | Src |
| --- | --- | --- | --- | --- | --- |
| A11Y-22 | "accessible" / "WCAG compliant" in copy, contract, or VPAT | **Scoped conformance claims** — version + level + page scope + date; conformance attaches to full pages and *every step* of a process; one failing step falsifies the sentence (↔ biz [[CLM-01]](business-marketing.md?plain=1#L158): a conformance claim is a claim-as-product with EAA/ADA force) | Could one failing page falsify this sentence? | B·g | wcag22-accessibility ch-7 |
| A11Y-23 | accessibility verified by automated scan only | **Scanner-pass is the floor of the floor** — automation detects a minority of criteria (<4.1% of pages actually conform); keyboard walk + AT pass on primary flows required (↔ biz [[PROD-12]](business-marketing.md?plain=1#L62): the worst-day user includes the AT user) | What did a human verify that axe cannot? | S·r | wcag22-accessibility ch-8 |
| A11Y-24 | screen states enumerated in a design/release review | **Every state accessible (state-matrix join)** — loading, empty, error, offline each keep focus management, announcements, and contrast; an accessible happy path with an inaccessible error state fails the process (↔ eng [[RLSE-04]](engineering.md?plain=1#L383) both fail when a state ships that was rendered but never designed) | Where does focus go when this errors? | S·r | wcag22-accessibility ch-1 + product-deploy-agents-fields ch-3 |
| A11Y-25 | accessibility work postponed as post-launch polish | **Retrofit costs more than the hedge** — the safe side (native elements, labels, contrast tokens) costs ~zero at write time and a rewrite later; same cost asymmetry as regulatory claims (↔ biz [[CLM-02]](business-marketing.md?plain=1#L159) both price the cheap precaution now against the expensive reversal later) | What does doing this now actually cost? | S·p | product-deploy-agents-fields ch-7 |
...
- **Contrast is designed upstream**: design [[COL-04]](design-aesthetics.md?plain=1#L76) (value architecture before hue) makes [[A11Y-01]](accessibility.md?plain=1#L41) pass by construction; [[A11Y-01]](accessibility.md?plain=1#L41) is the measurable readout that [[COL-04]](design-aesthetics.md?plain=1#L76) was actually followed. A team failing contrast in review skipped a design step, not an engineering one.
- **The state matrix has an accessibility dimension**: eng [[RLSE-04]](engineering.md?plain=1#L383) (undesigned state is a bug) × [[A11Y-24]](accessibility.md?plain=1#L84): a state isn't designed until its focus order and announcements are. PDA's UX lens and WCAG audit the same seven states from two threat models.
- **Claims discipline transfers whole**: [[A11Y-22]](accessibility.md?plain=1#L82) is [[CLM-01]](business-marketing.md?plain=1#L158)/[[CLM-04]](business-marketing.md?plain=1#L161) instantiated: "accessible" is an adjective-without-mechanism exactly like "encrypted", and the EAA/ADA give it the same enforcement asymmetry [[CLM-02]](business-marketing.md?plain=1#L159) describes.
- **Exclusion is a brand decision**: design [[BRND-07]](design-aesthetics.md?plain=1#L113) requires naming who a product is *not* for; an inaccessible flow silently makes that decision without anyone signing it. If an audience is to be excluded, that decision belongs in the brief, not in a `<div onclick>`.
- **Platform floor vs spec floor**: [[A11Y-14]](accessibility.md?plain=1#L64) (24px) vs platform HIG (44pt/48dp): always the stricter; the same "stricter-of-two-gatekeepers" logic as biz [[PROD-09]](business-marketing.md?plain=1#L59) channel contracts.
