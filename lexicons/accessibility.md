...
| A11Y-24<a name="a11y-24"></a> | screen states enumerated in a design/release review | **Every state accessible (state-matrix join)**: loading, empty, error, offline each keep focus management, announcements, and contrast; an accessible happy path with an inaccessible error state fails the process (↔ eng [[RLSE-04]](engineering.md#rlse-04) both fail when a state ships that was rendered but never designed) | Where does focus go when this errors? | S·r | [wcag22-accessibility ch-1](../SOURCES.md#src-wcag22-accessibility) + [product-deploy-agents-fields ch-3](../SOURCES.md#src-product-deploy-agents-fields) |
